...
public class ContadorApplet extends Applet implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == parar1Button) {
            hilo1.interrupt(); // Interrumpe el contador 1
            nombreHilo1Label.setText(hilo1.nombreHilo + " interr.");
        } else if (e.getSource() == parar2Button) {
            hilo2.interrupt(); // Interrumpe el contador 2
            ...
            if (!hilo1.isAlive()) hilo1 = new Hilo("H1", 2, this);
            if (!hilo2.isAlive()) hilo2 = new Hilo("H2", 1, this);
            hilo1.start();
            hilo2.start();
...
class Hilo extends Thread {
    ...
    @Override
    public void run() {
        while (!isInterrupted()) { // Revisar si el hilo ha sido interrumpido
            try {
